  ...
  if (accessDenied) {
    return (
      <div className="mx-auto max-w-lg p-8 text-center">
        ...
        <Link href="/platform/operations" className="mt-4 inline-block text-sm text-champagne font-bold">
          Back to dashboard
        </Link>
      </div>
    );
  ...
  if (loading || !branch || !branchId) {
    return (
      <div className="p-6 text-sm text-muted">
        {loading ? "LoadingΓÇª" : "Branch not found."}
      </div>
    );
  }
            ...
            {showForm && (
              ...
              >
                ...
                <button type="submit" className="btn-admin text-xs" disabled={submitting}>
                  {submitting ? "InvitingΓÇª" : "Invite staff"}
                </button>
              </form>
            )}
