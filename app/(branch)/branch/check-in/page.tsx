...
export default function CheckInScreen() {
  ...
  const { bookings, recordCashPayment, updateBookingStatus } = useManagerStore();
  ...
  const handleRecordCash = async (booking: ManagerBooking) => {
    ...
    if (res.ok) {
      ...
      toast.error(res.error ?? "We could not record the payment. Please try again.");
    }
  };
  ...
  const handleCheckIn = async (booking: ManagerBooking) => {
    ...
    if (res.ok) {
      ...
      toast.error(res.error ?? "We could not check the guest in. Record payment first if needed.");
    }
  };
